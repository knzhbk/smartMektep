...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        setupLayout()
        // После 5 сек должен открыться след VC, но не работает
        Timer.scheduledTimer(timeInterval: 2.0, target: self, selector: #selector(showSelectLAnguageVC), userInfo: nil, repeats: false)
    }
    
    @objc func showSelectLAnguageVC() {
